...
SimpleForm.setup do |config|
  ...
  config.label_text = ->(label, required, explicit) { "#{required} #{explicit ? label : label.to_s.titleize}" }
  ...
  config.wrappers :vertical_form, class: 'mb-3' do |b|
    ...
    b.use :hint, wrap_with: { class: 'form-text hint-text' }
  end
  ...
  config.wrappers :vertical_boolean, tag: 'fieldset', class: 'mb-3' do |b|
    ...
    b.wrapper :form_check_wrapper, class: 'form-check' do |bb|
      ...
      bb.use :hint, wrap_with: { class: 'form-text hint-text' }
    end
  end
